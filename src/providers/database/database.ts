...
export class DatabaseProvider {
  ...
  updateEmployee(emp: Emp){
    let data = [emp.name, emp.gender, emp.position, emp.surname, emp.email, emp.cnumber];
    return this.Db.executeSql(`UPDATE employee SET name = ?, gender = ?, position = ?, surname = ?, email = ?, cnumber = ? WHERE id = ${emp.id}`, data).then((data) =>{
      this.loadEmployees();
    })
   }
